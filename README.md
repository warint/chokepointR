...
`cp_resilience()` returns the profile; the composite **vulnerability
index** combines how much is at stake (trade value, dependent countries,
systemic risk) with how few alternatives exist.
...
<img src="man/figures/README-vulnerability-1.png" alt="" width="100%" />
...
indicators** of each chokepoint’s position in the real, weighted
country–chokepoint trade-dependency network (computed from Verschuur &
Hall 2025): unweighted and weighted betweenness (*brokerage*), degree
and weighted strength (*breadth* vs. *mass* of dependence), harmonic
centrality (*reachability*) and PageRank (*recursive influence*).
Centrality captures something oil volume does not: Hormuz carries a
fifth of the world’s oil yet brokers few country pairs, while
...
`cp_context()` adds a per-chokepoint profile — vessel transits, cargo
and vessel mix, trade shares, top users, local economic dependence and
rerouting cost:
...
Every figure is traceable through `cp_sources()`, with a `confidence`
flag so you can keep only authoritative numbers:
...
Filter with `cp_data()`, or map with `cp_map()` (interactive
**leaflet**):
